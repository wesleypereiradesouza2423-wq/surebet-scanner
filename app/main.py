...
async def perform_scan():
    if not ODDS_API_KEY or not engine: return {'ok':False,'reason':'missing ODDS_API_KEY or DATABASE_URL','db_error':DB_ERROR}
    async with httpx.AsyncClient(headers={'User-Agent':'SurebetScanner/1.0'}) as client:
        ...
        if row is None: return {'ok':False,'reason':'database unavailable','db_error':DB_ERROR}
